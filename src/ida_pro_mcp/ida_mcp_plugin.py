...
try:
    ...
    print(f"[MCP] 警告: 无法导入 IDA API: {e}")
    print("[MCP] 插件将在 IDA Pro 环境中正常工作")
...
DEFAULT_PORT = 13337
DEFAULT_ALLOW_PORT_OVERRIDE = True
...
# HTTP 配置
HTTP_TIMEOUT = None  # 无超时限制，适用于长时间运行的分析操作
...
def get_config_file_path():
    ...
    # 3. IDA插件目录（只在 IDA 环境中可用）
    try:
        plugin_dir = idaapi.idadir("plugins")
...
def load_config():
    ...
    default_config = {
        "host": DEFAULT_HOST,
        "port": DEFAULT_PORT,
        "allow_port_override": DEFAULT_ALLOW_PORT_OVERRIDE,
...
class Server:
    ...
    def start(self):
        ...
        try:
            ...
            try:
                ...
                self.allow_port_override = DEFAULT_ALLOW_PORT_OVERRIDE
                print("[MCP] 使用默认配置继续")
    ...
    def stop(self):
        ...
        try:
            ...
            # 优雅关闭服务器
            # 注意：shutdown() 必须从不同于 serve_forever() 的线程调用
            if self.server:
                try:
                    # 在单独的线程中调用 shutdown
    ...
    def _run_server(self):
        ...
        try:
            ...
            max_attempts = MAX_PORT_ATTEMPTS  # 最多尝试100个端口
            attempts = 0
            
            while attempts < max_attempts and self.running:
                try:
                    # 创建服务器实例（会自动调用 server_bind 和 server_activate）
                    ...
                    try:
                        ...
                        logger.error(f"服务器运行时错误: {serve_error}", exc_info=True)
                    finally:
                        self.running = False
                    ...
                    if "[WinError 10048]" in str(e) or "Address already in use" in str(e):
                        if self.allow_port_override and self.running:
                            ...
                            time.sleep(PORT_RETRY_DELAY)  # 短暂延迟，避免过快尝试
                        else:
...
# (IDASafety、call_stack、sync_wrapper、idaread、idawrite 已在文件开头定义)
...
def parse_address(address: str) -> int:
    ...
    Args:
        address: 地址字符串（如 "0x401000" 或 "4198400"）
        
    Returns:
        解析后的整数地址
        
    Raises:
        IDAError: 如果地址格式无效或超出范围
    """
    if not address:
    ...
    # 检查长度（防止过长输入）
    if len(address) > 20:
        raise IDAError(f"Address too long: {address}")
    ...
    try:
        ...
        # 提供更详细的错误信息
        for ch in address:
            if ch not in "0123456789abcdefABCDEFxX":
                raise IDAError(f"Invalid character '{ch}' in address: {address}")
        raise IDAError(f"Failed to parse address (missing 0x prefix?): {address}")
    
    # 验证地址范围
    if value < 0:
        raise IDAError(f"Address cannot be negative: {hex(value)}")
    
    return value

def get_function(address: int, *, raise_error: bool = True) -> Optional[Function]:
    """根据地址获取函数信息
    
    Args:
        address: 函数地址（整数）
        raise_error: 如果为 True，在找不到函数时抛出异常；
                    如果为 False，返回 None
    
    Returns:
        包含函数信息的字典，如果找不到且 raise_error=False 则返回 None
        
    Raises:
        IDAError: 当 raise_error=True 且找不到函数时
    """
    ...
    if fn is None:
        if raise_error:
            ...
            if seg is None:
                raise IDAError(f"Address {hex(address)} is not in any segment")
            elif not (seg.perm & idaapi.SEGPERM_EXEC):
                raise IDAError(f"Address {hex(address)} is in non-executable segment {idaapi.get_segm_name(seg)}")
            else:
                raise IDAError(f"No function found at address {hex(address)} (segment: {idaapi.get_segm_name(seg)})")
        return None
...
if _IDA_AVAILABLE:
    ...
    # 占位符类
    class my_modifier_t:
        def __init__(self, var_name: str, new_type):
            pass
        def modify_lvars(self, lvars):
            return False
            ...
            try:
                ...
                if hasattr(self.server, 'running') and self.server.running:
                    ...
                    try:
                        ...
                        print(f"[MCP] 警告: 停止旧服务器时出错: {str(e)}")
